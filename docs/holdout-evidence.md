...
`cowbot.monitor`, `cowbot.report`, `cowbot.scenario`, and `cowbot.stream`
blocked. All three captures must match the validated in-process preflight
contract. The unwrapped machine-readable capture is
...
- no holdout result file, visual prefix, case execution, row decoding, or
  adverse-row reduction;
...
that an external executor would pass the protocol, or that the pre-registered
acceptance thresholds have been met.
